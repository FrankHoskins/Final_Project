...
function fetchTemperatures() {
	...
	httpRequest.onload = (function() {
		...
		if (responseObj.status) { apiError(); return; }

		httpRequest.abort(); httpRequest.open("GET", responseObj.properties.forecastOffice, true);
		httpRequest.onload = (function() {
			if (httpRequest.status != 200) { requestError(); return; }
			var stationObj = JSON.parse(httpRequest.responseText); if (stationObj.status) { apiError(); return; }
			...
			for (let i = 0; i < stationObj.approvedObservationStations.length; i += 4) {
				...
				if (stationRequest.status != 200) { console.warn("Station Error."); console.log(stationRequest); return; }
				...
				localRequest.open("GET", "https://api.weather.gov/zones/forecast/" + tempWeather.id + "/observations?limit=1", false); localRequest.send(null);
...
window.onload = function() {
	...
	forecastRequest.onload = (function() {
		if (forecastRequest.status != 200) { console.log(forecastRequest); alert("Unable to retreive forecast."); return; }
	...
	getClass("radarOption")[0].addEventListener("click", function() {
		...
		radarMap.overlayMapTypes.clear(); clearInterval(activeweatherLoop);
		if (temperatureMarkers.length == 0) {
			for (let i = 0; i < temperatureTiles.length; i++) {
				let tempMarker = new google.maps.Marker({
					position: {lat: temperatureTiles[i].Position[1], lng: temperatureTiles[i].Position[0]},
					map: radarMap,
	...
	getClass("radarOption")[1].addEventListener("click", function() {
		...
		for (let i = 0; i < temperatureMarkers.length; i++) { temperatureMarkers[i].setMap(null); }
...
function loadMap() {
	...
	for (let i = 10; i > 1; i += -1) { timestamps.push("900913-m" + i * 5 + "m"); };
	timestamps.push("900913-m05m"); timestamps.push("900913");
